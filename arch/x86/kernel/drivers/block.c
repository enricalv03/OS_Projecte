 ...
 * If ATA init or identify fails, we simply skip registration - no crash. */
void block_init(void) {
    ...
    /* Initialize ATA if available (returns -1 if no drive present) */
    if (ata_init() != 0) {
        return;
    }
    
    {
        ...
        static unsigned short identify_buffer[256];
