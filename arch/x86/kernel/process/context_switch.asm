...
global context_restore_from_irq_asm
...
context_restore_asm:
    ...
    ; Restore EFLAGS; always set IF so interrupts are enabled
...
; =============================================================================
; context_restore_from_irq_asm(pcb_t* pcb)
; =============================================================================
; Restores a process that was preempted from a timer IRQ.
; The PCB's ESP must point to the IRQ handler's saved frame (pushad, ds,es,fs,gs).
; We pop that frame and iret to complete the "return" from the interrupt.
; This does NOT return to the caller -- it jumps to the restored process.
; =============================================================================
context_restore_from_irq_asm:
    mov eax, [esp + 4]         ; EAX = pcb_t* argument
    test eax, eax
    jz .restore_irq_fail

    ; Load stack pointer from PCB (points to our saved frame)
    mov esp, [eax + PCB_ESP]
...
.skip_save:
    ...
    ; Restore EFLAGS; always set IF so interrupts are enabled when we resume
    push dword [eax + PCB_EFLAGS]
    or dword [esp], 0x200
    popfd
